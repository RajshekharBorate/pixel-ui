...
@Component({
  selector: 'docs-date-range-reactive-form-example',
...
export class DateRangeReactiveFormExample {
  ...
  protected submit(): void {
    if (this.form.invalid) {
      ...
      this.submitted.set('Form is invalid — both dates are required.');
      return;
    }
    const start = this.form.controls.start.value;
    const end = this.form.controls.end.value;
    this.submitted.set(
      `Submitted: ${start?.toDateString() ?? '—'} → ${end?.toDateString() ?? '—'}`,
    );
  }
}
