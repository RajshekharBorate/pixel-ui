...
- Registers `ensureLineChart()` at module load (includes optional dataZoom).
- `showValues="auto"` hides labels when dense.
- `dataZoom` enables inside / slider / both; listen to `dataZoomChange`.
...
- Removed `brush` input, `brushEnd` output, and brush toolbox registration.
  Use `dataZoom` for range navigation.
