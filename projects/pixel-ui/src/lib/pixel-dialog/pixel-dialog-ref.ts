...
/**
 * Handle to a dialog opened via {@link PixelDialogService.open}. Inject it in the opened component
 * to close the dialog and pass a result back to the caller; the caller observes the result through
 ...
 */
export class PixelDialogRef<R = unknown, T = unknown> {
  /** The opened component instance. Populated by the service before the dialog renders. */
  componentInstance: T | null = null;

  /**
   * True when {@link PixelDialogService.open} refused the call (`config.requires` denied).
  ...
  /** Close the dialog, optionally passing a result emitted by {@link afterClosed}. */
  close(result?: R): void {
    if (this.settled) {
      return;
    }
  ...
  /** Emits once, after the dialog has finished opening, then completes. */
