...
- **`config.requires`:** when authorization denies, `open()` returns a ref with `accessDenied: true`, no overlay, and `afterClosed` completes. Check `ref.accessDenied` before treating the dialog as opened.
