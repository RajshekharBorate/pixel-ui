...
- Panel colors, radii, and spacing use `--pixel-autocomplete-*` custom properties.
- Option hover/selected states use `color-mix()` against system tokens.
- Field chrome inherits `pixel-input` border, focus ring, and label tokens.
- Dark mode via `prefers-color-scheme: dark` and `[data-theme]` / enterprise theme roots.
